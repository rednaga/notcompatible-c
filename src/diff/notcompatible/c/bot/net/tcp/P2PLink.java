...
public class P2PLink extends TCPSocket {
    ...
    public void connect(InetSocketAddress newSocketAddress) {
        try {
            LOGGER.info(" [!] Attempting to make a P2P connection to : " + newSocketAddress.getHostName() + "/"
                            + newSocketAddress.getPort());
            isIncoming = false;
    ...
    public void onConnect(SelectionKey key) throws IOException {
        ...
        if (hr != null) {
            ...
            hr = owner.hubList.getByIpAndPort(channel.socket().getInetAddress().getHostAddress(), channel.socket()
                            .getPort());
            if (hr == null) {
                hr = owner.hubList.add();
            }
        ...
        if (isIncoming || (status != LinkStatus.KEY_EXCHANGE_DONE) || (readBuffer.size <= 4)) {
            if (((status != LinkStatus.OFFLINE) && isIncoming) || (readBuffer.size <= 4)) {
                ...
                if (dataLength < readBuffer.size) {
                    LOGGER.warning(" [!] P2P data claims we received " + dataLength
                                    + " bytes of data, less than what the read buffer says we have - exiting!");
                    close();
                } else {
