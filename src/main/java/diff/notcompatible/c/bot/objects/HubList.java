...
public class HubList {
    ...
    public void loadFromRaw(byte[] buffer) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            item.ip = String.valueOf(tmp[0] & 0xFF) + "." + String.valueOf(tmp[1] & 0xFF) + "."
                            + String.valueOf(tmp[2] & 0xFF) + "." + String.valueOf(tmp[3] & 0xFF);
            // Get port
            System.arraycopy(buffer, (i * 10) + 4, tmp, 0, 4);
            item.port = ((tmp[0] & 0xFF) << 8) | (tmp[1] & 0xFF);
            System.arraycopy(buffer, (i * 10) + 6, tmp, 0, 4);
            // Get last connect
            item.lastConnect = (((((tmp[3] & 0xFF) << 24) | ((tmp[2] & 0xFF) << 16)) | ((tmp[1] & 0xFF) << 8)) | ((tmp[0] & 0xFF) * 1000));
        }
    }
    ...
    public void loadFromPacket(Packet packet) {
        ...
        for (int i = 0; i < packet.getCount(); i++) {
            ...
            if (hublistItemPacket.tag.equals("Item")) {
                ...
                if ((ipPacket != null) && (portPacket != null)
                                && (getByIpAndPort(ipPacket.asIP(), portPacket.asPort()) == null)) {
    ...
    public HubListItem getNextToConnect() {
        ...
        for (int i = 0; i < count(); i++) {
            item = (HubListItem) list.getObject(i);
            if (!item.used && (item.status == -1) && ((item.lastTested + 600000L) <= currentTime)
                            && (item.connectCount < 10)) {
                return item;
            }
        }
